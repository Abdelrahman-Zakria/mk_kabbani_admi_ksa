...
import 'package:mk_kabbani_admin/models/product_addons.dart';
import 'package:mk_kabbani_admin/models/product_attribute.dart';
import 'package:mk_kabbani_admin/models/product_variation.dart';
import 'package:mk_kabbani_admin/models/store.dart';
import 'package:mk_kabbani_admin/models/tag.dart';
import 'package:mk_kabbani_admin/models/vendor_admin_var.dart';
...
class Product {
    ...
    try {
      ...
      if (isVariableProduct && parsedJson['variation_products'] != null) {
        try {
          ...
          log('variation_products_error ${parsedJson['id']} $e');
        }
      }
      metaData = List<Map<String, dynamic>>.from(parsedJson['meta_data']);
    } catch (e, trace) {
      log("trace : $trace");
      log('🔴 Get product $name :${e.toString()}');
    }
  }

  Product.fromShopify(Map<String, dynamic> json) {
    log("price regular : ${json['variants']['edges'][0]['node']['priceV2']}");
    try {
      var priceV2 = json['variants']['edges'][0]['node']['priceV2'];
      var compareAtPrice =
      json['variants']['edges'][0]['node']['compareAtPriceV2']['amount'];
  ...
  static String cleanProductID(productString) {
    ...
    log("productString : $productString");
  ...
  //   } catch (err) {
  //     log('err when parsed json Listing $err');
  //   }
  // }
